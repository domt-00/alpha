#!/usr/bin/env bash
# Resumes the DeepSeek experiment from VD2 (XOR + VD1 already completed).
#
# Usage (from DT Study directory):
#   nohup bash scripts/run-deepseek-vd2-onwards.sh > logs/deepseek-experiment.log 2>&1 &
...
log "=== DeepSeek-V3 experiment (VD2 onwards, max_ica=$MAX_ICA) ==="
log "    Benchmark : $BENCH (3 items, 9 people, 3 setups)"
log "    XOR + VD1 : already done — skipping"
