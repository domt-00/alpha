#!/usr/bin/env python3
"""NVD-Ensemble runner — median of N independent LLM calls per bundle valuation."""
...
from alpha.auctions.ceca_qllm_a_ensemble import CECA_QLLM_A_Ensemble_Proxy_Factory
...
def main():
    ...
    parser.add_argument("--ensemble_size",             type=int, default=3,
                        help="Number of independent LLM samples per bundle valuation (median taken).")
    parser.add_argument("--compress_description", action="store_true", default=False)
    args = parser.parse_args()

    token_tracker.set_context(benchmark=args.benchmark, stage="PROXY-NVD-ENSEMBLE")
    ...
    def make_factory():
        return CECA_QLLM_A_Ensemble_Proxy_Factory(
    ...
    overall_log_df["Proxy"] = "Proxy-NVD-Ensemble"
    ...
    overall_log_df["Proxy-ensemble_size"]             = args.ensemble_size
    ...
    log_filename = os.path.join(out_dir, f"log_Proxy-NVD-Ensemble_{timestamp}.csv")
