#!/usr/bin/env python3
"""NVD-Prune runner — trend pruning only, no bundle restriction."""
...
from alpha.auctions.ceca_qllm_a_r import CECA_QLLM_A_Prune_Factory
...
def main():
    ...
    token_tracker.set_context(benchmark=args.benchmark, stage="PROXY-NVD-PRUNE")
    ...
    def make_factory():
        return CECA_QLLM_A_Prune_Factory(
    ...
    overall_log_df["Proxy"] = "Proxy-NVD-Prune"
    ...
    log_filename = os.path.join(out_dir, f"log_Proxy-NVD-Prune_{timestamp}.csv")
    overall_log_df.to_csv(log_filename, index=False)
    print(f"[INFO] Saved to {log_filename}")

    for r in rows:
        print(f"  Setup {r['setup_index']}: welfare={r['total_auction_value']}  pruned={r['bundles_pruned']}")
