#!/usr/bin/env bash
# Run all 5 proxies on the 3 mini benchmarks using Cerebras (gpt-oss-120b).
# Usage (from DT Study directory):
#   nohup bash scripts/run-mini-proxies-cerebras.sh > logs/mini-pipeline-cerebras.log 2>&1 &
...
log "Starting 3-item mini proxy runs (Cerebras / gpt-oss-120b)"

for BENCH in electronics-mini-cerebras electronics-mini-specs-cerebras electronics-mini-pdf-cerebras; do
