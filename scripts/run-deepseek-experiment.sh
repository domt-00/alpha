#!/usr/bin/env bash
# DeepSeek-V3 experiment — all 5 proxies on electronics-mini (3 items, 9 people, 3 setups).
# Results go to data/electronics-mini-deepseek-logs/ (separate from all prior runs).
# Prints token usage and USD cost at the end.
#
# Usage (from DT Study directory):
#   nohup bash scripts/run-deepseek-experiment.sh > logs/deepseek-experiment.log 2>&1 &
