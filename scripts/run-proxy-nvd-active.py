#!/usr/bin/env python3
"""NVD-Active runner — confidence-driven bundle refresh (active-learning style)."""
...
from alpha.auctions.ceca_qllm_a_active import CECA_QLLM_A_Active_Proxy_Factory
...
def process_scenario(benchmark, scenario_code, setup_index, proxy_factory, timestamp, max_iterations=None):
    ...
    try:
        ...
        for proxy in proxies:
            ...
            for c in getattr(proxy, "bundle_confidence", {}).values():
                counts[c] = counts.get(c, 0) + 1
            confidence_counts.append(counts)
...
def main():
    ...
    token_tracker.set_context(benchmark=args.benchmark, stage="PROXY-NVD-ACTIVE")
    ...
    def make_factory():
        return CECA_QLLM_A_Active_Proxy_Factory(
    ...
    overall_log_df["Proxy"] = "Proxy-NVD-Active"
    ...
    log_filename = os.path.join(out_dir, f"log_Proxy-NVD-Active_{timestamp}.csv")
    overall_log_df.to_csv(log_filename, index=False)
    print(f"[INFO] Saved to {log_filename}")

    for r in rows:
        print(f"  Setup {r['setup_index']}: welfare={r['total_auction_value']}  confidence={r['confidence_counts']}")
