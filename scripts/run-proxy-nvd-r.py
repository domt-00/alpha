#!/usr/bin/env python3
"""
NVD-R (Restricted) proxy runner.
Identical to run-proxy-nvd.py but uses CECA_QLLM_A_R_Proxy_Factory,
which adds bundle restriction and trend pruning to reduce wasted LLM calls.
"""
...
from alpha.auctions.ceca_qllm_a_r import CECA_QLLM_A_R_Proxy_Factory
...
def process_scenario(benchmark, scenario_code, setup_index, proxy_factory, timestamp, max_iterations=None):
    ...
    try:
        ...
        row = {
            ...
            "bundles_pruned": [len(getattr(proxy, "_pruned_bundles", set())) for proxy in proxies],
            "reasonable_codes": [list(getattr(proxy, "_reasonable_codes", set()) or set()) for proxy in proxies],
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run NVD-R (bundle-restricted) auction proxy.")
    ...
    parser.add_argument("--prune_percentile",        type=float, default=0.25,
                        help="Fraction of lowest-value bundles to exclude after each refresh (default 0.25).")
    ...
    token_tracker.set_context(benchmark=args.benchmark, stage="PROXY-NVD-R")
    ...
    def make_factory():
        return CECA_QLLM_A_R_Proxy_Factory(
    ...
    overall_log_df["Proxy"] = "Proxy-NVD-R"
    ...
    overall_log_df["CompressDescription"] = args.compress_description

    out_dir = f"data/{args.benchmark}-logs"
    os.makedirs(out_dir, exist_ok=True)
    log_filename = os.path.join(out_dir, f"log_Proxy-NVD-R_{timestamp}.csv")
    overall_log_df.to_csv(log_filename, index=False)
    print(f"[INFO] Processing complete. Logs saved to {log_filename}")

    if rows:
        for r in rows:
            print(f"  Setup {r['setup_index']}: welfare={r['total_auction_value']}  "
                  f"pruned={r['bundles_pruned']}  "
                  f"reasonable_codes={r['reasonable_codes']}")
