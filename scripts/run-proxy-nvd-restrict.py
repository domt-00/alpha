#!/usr/bin/env python3
"""NVD-Restrict runner — bundle restriction only, no trend pruning."""
...
from alpha.auctions.ceca_qllm_a_r import CECA_QLLM_A_Restrict_Factory
...
def main():
    ...
    token_tracker.set_context(benchmark=args.benchmark, stage="PROXY-NVD-RESTRICT")
    ...
    def make_factory():
        return CECA_QLLM_A_Restrict_Factory(
    ...
    overall_log_df["Proxy"] = "Proxy-NVD-Restrict"
    ...
    log_filename = os.path.join(out_dir, f"log_Proxy-NVD-Restrict_{timestamp}.csv")
    overall_log_df.to_csv(log_filename, index=False)
    print(f"[INFO] Saved to {log_filename}")

    for r in rows:
        print(f"  Setup {r['setup_index']}: welfare={r['total_auction_value']}  codes={r['reasonable_codes']}")
