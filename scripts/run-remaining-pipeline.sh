#!/usr/bin/env bash
# Continues the pipeline from STEP 3 (ELECTRONICS-REVIEWS benchmark).
# STEP 1 (ELECTRONICS FullPersons) and STEP 2 (ELECTRONICS-SPECS) are already done.
#
# Usage (from DT Study directory):
#   nohup bash scripts/run-remaining-pipeline.sh >> logs/pipeline.log 2>&1 &
