#!/usr/bin/env bash
# DeepSeek-V3 — all 5 proxies on electronics (6 items, 3 people, 3 setups).
#
# Usage (from DT Study directory):
#   nohup bash scripts/run-deepseek-electronics.sh > logs/deepseek-electronics.log 2>&1 &
#   tail -f logs/deepseek-electronics.log
...
log "=== DeepSeek-V3 — electronics (6 items, 3 people, 3 setups) ==="
log "    Benchmark : $BENCH"
log "    Model     : deepseek-chat (DeepSeek-V3)"
log "    Pricing   : \$0.27/M input | \$1.10/M output"
log "    Max ICA   : $MAX_ICA iterations"
