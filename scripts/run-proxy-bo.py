#!/usr/bin/env python3
"""CECA-BO runner — Gaussian Process / Bayesian-optimization proxy.

Unlike the VD1/VD2/H/NVD family, this proxy makes no per-bundle LLM calls at
all: bundle values come from a GP fit on a handful of real person queries.
No --env_path LLM provider is needed for the proxy's own reasoning (only the
FullPerson simulated bidder still needs an LLM to answer value queries).
"""
...
from alpha.auctions.ceca_bo import CECA_BO_Proxy_Factory
...
def main():
    ...
    token_tracker.set_context(benchmark=args.benchmark, stage="PROXY-BO")
    ...
    log_filename = os.path.join(out_dir, f"log_Proxy-BO_{timestamp}.csv")
    overall_log_df.to_csv(log_filename, index=False)
    print(f"[INFO] Saved to {log_filename}")

    for r in rows:
        print(f"  Setup {r['setup_index']}: welfare={r['total_auction_value']}  interactions={r['human_interactions']}")
