#!/usr/bin/env bash
# NVD-SR pilot experiment — Llama 3.3 70B via Cerebras
#
...
# Model     : llama3.3-70b (Cerebras free tier — ~60-100K TPM, much faster than Groq)
# Purpose   : test whether NVD-SR skips questions without hurting efficiency
#
# Usage (from DT Study directory):
#   nohup bash scripts/run-nvd-sr-pilot-cerebras.sh > logs/nvd-sr-pilot-cerebras.log 2>&1 &
#   tail -f logs/nvd-sr-pilot-cerebras.log
...
CAP=20
MIN_ITER=5          # match NUM_Q so all question slots are offered before convergence
...
log "=== NVD-SR Pilot Experiment (Cerebras) ==="
log "    Benchmark  : $BENCH  (3 people, 1 setup, 3 items)"
log "    Model      : gpt-oss-120b via Cerebras (~60-100K TPM)"
log "    Env        : $ENV"
...
print(f"  Model: llama3.3-70b (Cerebras)")
print("=" * 65)
