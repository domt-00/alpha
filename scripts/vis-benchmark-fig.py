...
def main():
    ...
    if missing:
        assert False, "Please be sure to run the following proxies against the benchmark before calling vis-benchmark-fig.py"
    ...
    # fig 2
    fig2df = normalized_df[normalized_df["Proxy"].isin([
        "Proxy-XOR", "Proxy-VD1", "Proxy-VD2", "Proxy-NVD"
    ])]
    ...
    fig.savefig(f"{args.outprefix}_fig2.png", dpi=300, bbox_inches='tight')


    # fig 3
    fig3adf = normalized_df[normalized_df["Proxy"].isin([
        "Proxy-XOR", "Proxy-H"
    ])]
    ...
    # Save the figure via Matplotlib
    fig.savefig(f"{args.outprefix}_fig3a.png", dpi=300, bbox_inches='tight')
