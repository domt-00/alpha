...
def process_scenario(benchmark, scenario_code, setup_index, proxy_factory, timestamp, max_iterations=None):
    ...
    Args:
        ...
        proxy_factory: The CECA_HybridXOR_Proxy factory instance.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run CECA_HybridXOR_Proxy Auction Simulation with environment variables."
    )

    # Match the style from run-proxy-xor
    ...
    # Additional parameters for HybridXOR proxy
    ...
    # Prepare the HybridXOR proxy factory with arguments
    def hybrid_proxy_factory_constructor():
        return CECA_HybridXOR_Proxy_Factory(
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, initializer=_init_worker, initargs=(args.env_path,)) as executor:
        ...
        for (scenario_code, setup_index) in tasks:
            ...
            # but pass the instance to each worker (so each worker has its own instance).
            proxy_factory = hybrid_proxy_factory_constructor()
            futures.append(
                executor.submit(
                    process_scenario,
                    args.benchmark,
